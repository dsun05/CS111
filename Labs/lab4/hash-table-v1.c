...
struct hash_table_v1 *hash_table_v1_create()
{
	...
	int ret;
	if ((ret = pthread_mutex_init(&hash_table->lock, NULL)) != 0) {
		free(hash_table);
		exit(ret);
	}
...
{
    int ret;
    if ((ret = pthread_mutex_lock(&hash_table->lock)) != 0) {
        exit(ret);
    }
	...
	if (list_entry != NULL) {
        ...
        if ((ret = pthread_mutex_unlock(&hash_table->lock)) != 0) {
            exit(ret);
        }
		return;
	}
...
void hash_table_v1_destroy(struct hash_table_v1 *hash_table)
{
	...
	if ((ret = pthread_mutex_destroy(&hash_table->lock)) != 0) {
        exit(ret);
	}
