...
void write_superblock(int fd) {
	...
	superblock.s_log_block_size = BLOCK_SIZE >> 1024;		/* 1024 */
	superblock.s_log_frag_size = BLOCK_SIZE >> 1024;		/* 1024 */
...
void write_root_dir_block(int fd)
{
	...
	current_entry.inode = EXT2_ROOT_INO;
	...
	parent_entry.inode = EXT2_ROOT_INO;
...
void write_lost_and_found_dir_block(int fd) {
	off_t offset = BLOCK_OFFSET(LOST_AND_FOUND_DIR_BLOCKNO);
	...
	current_entry.inode = LOST_AND_FOUND_INO;
	dir_entry_write(current_entry, fd);
	bytes_remaining -= current_entry.rec_len;
